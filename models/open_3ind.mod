...
#subject to R1{i in V,j in V: i<>j}: sum{k in K}x[i,j,k]=1;
## Leave origin depot 
subject to R2:sum{k in K}y[0,k]=l;
#subject to R2:sum{k in K, i in V}x[0,i,k]=1;
## Arrive at destination depot
subject to R21:sum{k in K}y[n+1,k]=l;
#subject to R21:sum{k in K, i in V}x[i,n+1,k]=1;
## Tours cannot terminate at origin node
subject to R22:sum{k in K,i in H}x[i,0,k]=0;
...
#subject to R25:sum{k in K}x[n+1,0,k]=0;
